...
const ServiceWorkerWrapper = ({
	...
	const registerServiceWorker = useCallback(async () => {
		try {
			const registration =
				...
				(await navigator.serviceWorker.register(`${publicUrl}${publicServiceWorkerDest}`));
			...
			registration.addEventListener('updatefound', () => {
				...
				worker?.addEventListener('statechange', () => {
					if (worker.state === 'installed') {
						navigator.serviceWorker.controller ? onUpdated?.() : onInstalled?.();
					}
				});
			});
		} catch (err) {
	...
	}, [publicUrl, publicServiceWorkerDest, onError, onWaiting, onInstalled, onUpdated]);
	...
	const register = useCallback(() => {
		if ('serviceWorker' in navigator) {
			if (new URL(publicUrl, window.location.href).origin !== window.location.origin) {
				console.warn('Service worker cannot be registered because PUBLIC_URL is on a different origin from page.');
				return;
			}

			isLocalhost ? checkServiceWorker() : registerServiceWorker();
		}
